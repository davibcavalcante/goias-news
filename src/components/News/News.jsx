...
const News = ({ title, link, image, pubDate, isMain = false, invert = false }) => {
  return (
    <article className={`overflow-hidden col-span-2 ${isMain ? "col-span-2" : "md:col-span-1"}`}>
      <Link to={link} target="_blank" rel="noopener noreferrer" className="flex items-stretch justify-between">
        ...
        <div className={`flex flex-col w-1/2 p-4 sm:text-2xl ${isMain ? "text-xl md:text-2xl" : "text-lg"}`}>
          <h1 className={`text-main font-title flex-1 ${isMain ? "lg:text-4xl font-semibold" : "" }`}>{title}</h1>
